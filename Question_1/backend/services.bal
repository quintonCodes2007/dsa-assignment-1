...
service /assets on new http:Listener(8080) {
    ...
    // ===== GET /assets (with optional institution filter) =====
    ...
    resource function post .(@http:Payload Asset asset) returns http:Response {
        ...
        if assets.hasKey(tag) {
            io:println("Asset creation failed: tag ", tag, " already exists");
            return errorResponse(409, "Asset already exists");
        }
        ...
        io:println("Asset ", newAsset.name, " created with tag ", tag);
        return successResponse(201, "Asset created");
    }
    ...
    // ===== GET /assets/overdue (maintenance alerts) =====
    resource function get overdue() returns Asset[] {
        ...
        io:println(overdueList.length(), " overdue asset(s) found");
        return overdueList;
    }
...
// ===== Schedules =====
    ...
    // ===== Loans =====
    resource function get loans() returns Loan[] {
        Loan[] allLoans = loans.toArray();
        io:println(allLoans.length(), " loan(s) retrieved");
        return allLoans;
    }
...
// ===== Bookings =====
    resource function get bookings() returns Booking[] {
        Booking[] allBookings = bookings.toArray();
        io:println(allBookings.length(), " booking(s) retrieved");
        return allBookings;
    }
    ...
    resource function delete bookings/[string bookingId]() returns http:Response {
        Booking? booking = bookings[bookingId];

        if booking is Booking {
            _ = bookings.remove(bookingId);
            ...
            io:println("Booking ", bookingId, " cancelled");
            return successResponse(200, "Booking cancelled");
        }

        io:println("Booking cancellation failed: ", bookingId, " not found");
